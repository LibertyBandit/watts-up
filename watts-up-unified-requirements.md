...
### Phase B: Inhibit 4 content controls, add wu-interval (items 3.1, 3.2.1)

*Shipped 2026-09-20.*

**Inhibited for new reports** (item 3.1): `wu-preparedby`, `wu-intro`, `wu-general-notes`,
`wu-compliance` are no longer generated by `wuContentControlList()` (Phase 5's blank-document
generator, driving "Create New Report") — 11 controls now instead of 14. Scoped exactly to "new
reports" as asked: `fillWattsUpDocx()` (used by Sync, Connect Existing, and the legacy download-
only fallback) is completely untouched — any existing document that already has one of these 4
controls (the old bundled template, or a user's own customized file) still gets it filled normally
on sync. The underlying `state.meta` fields aren't touched either; only whether a *freshly
generated* document includes a content control for them changed. `wu-approvedby` (also in the
spec's inhibit list) required no change — it was never in this generator to begin with; it's a
tag from the original template vocabulary (§17.3) that was never wired into the active fill list.

**New: `wu-interval`** (item 3.2.1): added to both `fillWattsUpDocx()` (so an existing document
that gains this control — most naturally a Phase-5-generated one — fills correctly on sync) and
`wuContentControlList()` (so newly generated documents include it). Reads `state.meta.interval`,
the field moved into the header in Phase A.

Verified live: a freshly generated document now has exactly 11 controls (confirmed by tag list),
none of the 4 inhibited ones present, `wu-interval` present and correctly filled ("Continuous");
the legacy full-template `generateDocx()` path still fills `wu-preparedby`/`wu-intro` exactly as
before (unchanged), correctly reports `wu-interval` as absent there (expected — the old template
predates this tag, non-fatally noted via the existing missing-tag mechanism, not a bug). Clean
